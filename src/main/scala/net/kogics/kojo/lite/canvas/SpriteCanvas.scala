...
import net.kogics.kojo.util.{FileChooser, Utils}

import java.awt.{Color, Dimension, GradientPaint, Paint}
import java.awt.event._
import java.awt.geom.Point2D
import java.util.concurrent.Future
import java.util.logging.Logger
import javax.swing.{JCheckBoxMenuItem, JMenuItem, JPopupMenu}
...
class SpriteCanvas(val kojoCtx: core.KojoCtx) extends PSwingCanvas with SCanvas {
  ...
  val AxesColor = canvasAxesColor
  val GridColor = canvasGridColor
  val TickColor = canvasTickColor
  val TickLabelColor = canvasTickLabelColor
  val TickIntegerLabelColor = canvasTickIntegerLabelColor
  ...
  def cbounds = Utils.runInSwingThreadAndWait {
    getCamera.getViewBounds()
  }
