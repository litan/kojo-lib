...
trait CoreBuiltins extends Rationals {
  ...
  def textExtent(text: String, fontSize: Int, fontName: String = null) = Utils.runInSwingThreadAndWait {
    val tnode = Utils.textNode(text, 0, 0, DCanvas.camScale, fontSize, if (fontName == null) None else Some(fontName))
  ...
  // For younger kids
  def clr() = { DCanvas.clear(); DCanvas.turtle0.invisible() }
  def nt: LoTurtle = nt(0, 0)
  def nt(x: Double = 0, y: Double = 0): LoTurtle = {
    nt(DCanvas.newTurtle(x, y, "/images/blue-turtle32.png"))
  }
